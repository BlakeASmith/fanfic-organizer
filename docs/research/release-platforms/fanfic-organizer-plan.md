# Plan: list Fanfic Organizer in Calibre Get plugins

Goal: **Preferences → Get plugins to enhance calibre** shows **Fanfic Organizer**, Install works, later GitHub tags also show as in-app updates.

GitHub Releases stay. They are not the in-app source (see [calibre-get-plugins.md](calibre-get-plugins.md)).

## Already true

- Fat zip: `python makeplugin.py zip` / CI on `v*` tags → `fanfic-organizer.zip`.
- `calibre-plugin/__init__.py`: `name = 'Fanfic Organizer'`, `author = 'Emily'`, `version = __version__`, platforms `windows`/`osx`/`linux`, `minimum_calibre_version = (5, 0, 0)`, `InterfaceActionBase` from `calibre.customize`.
- Empty `plugin-import-name-fanfic_organizer.txt` (multi-file import).
- Unique catalog name (261 plugins, 2026-08-23: no Fanfic Organizer / AO3-named GUI plugin).
- Legacy **AO3 Scraper** already stripped on checkout install — use `Uninstall: AO3 Scraper, Wranglekit` on the index so Get plugins does the same.
...
| 2 | Support thread `[GUI Plugin] Fanfic Organizer` with **one** zip on **post 1** | Scraper takes first `attachment.php` `*.zip` |
...
| 5 | Prove `plugins_mirror.py fanfic-organizer.zip` parses | Else you sit in the sticky and never appear in Calibre ([2026 case](https://www.mobileread.com/forums/showthread.php?t=374096)) |
...
Expect JSON with `name: Fanfic Organizer`, `author: Emily`, `version` matching the tag, `minimum_calibre_version: [5,0,0]`. Check zip size ≪ 20 MB.
...
3. Title: **`[GUI Plugin] Fanfic Organizer`**
4. Body: draft below. Attach **`fanfic-organizer.zip` only** (same bytes as the GitHub release you want as “current”).
...
the multi-site story downloader; Fanfic Organizer is search → Calibre columns → tag
...
Install
1. Preferences → Get plugins to enhance calibre → Fanfic Organizer → Install
...
Source, issues, GitHub zip: https://github.com/BlakeASmith/fanfic-organizer
...
from Get plugins once Fanfic Organizer is listed (index Uninstall takes care of it).
...
```text
Please add Fanfic Organizer to the Index of plugins (GUI Plugins), alphabetically.

[*][URL="https://www.mobileread.com/forums/showthread.php?t=THREAD_ID"]Fanfic Organizer[/URL] Search AO3, download native EPUBs, wrangle tags, generate covers, and import into a Calibre library. History: Yes; Uninstall: AO3 Scraper, Wranglekit

- Link text is the internal plugin name (Fanfic Organizer).
...
Optional: PM Kovid to mention Fanfic Organizer in Calibre’s next What’s New.
...
2. Wait hours (up to ~1 day). Check `Last-Modified` on [plugins.json.bz2](https://plugins.calibre-ebook.com/plugins.json.bz2) and search [plugins.calibre-ebook.com](https://plugins.calibre-ebook.com/) for Fanfic Organizer.
3. In Calibre: Get plugins → filter **Not installed** → Fanfic Organizer → Install → restart.
4. If the sticky has you but Calibre does not: re-run `plugins_mirror.py fanfic-organizer.zip`. Ask in the thread; do not add a second zip to post 1.
...
1. Cut the GitHub release as today (`fanfic-organizer.zip`, version bump, CHANGELOG).
...
**Do not rename `name = 'Fanfic Organizer'`** after listing. A rename needs a new index row + `Uninstall: Fanfic Organizer` (see DeACSM → ACSM Input). Index `Uninstall` should list leftover **AO3 Scraper** and **Wranglekit**.

## Checklist (copy)

- [ ] `plugins_mirror.py fanfic-organizer.zip` prints metadata
- [ ] MobileRead account
- [ ] Thread `[GUI Plugin] Fanfic Organizer` + post-1 zip + reserved post 2
- [ ] PM mod with `[*][URL=…]Fanfic Organizer[/URL] … Uninstall: AO3 Scraper, Wranglekit`
