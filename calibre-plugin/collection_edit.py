    ...
    *,
    ...
    if items:
        name, ok = QInputDialog.getItem(
            parent, 'Fanfic Organizer', prompt, items, start, True
        )
    else:
        name, ok = QInputDialog.getText(parent, 'Fanfic Organizer', prompt)
    if not ok:
        return None
    name = str(name or '').strip()
    if not name:
        error_dialog(parent, 'Fanfic Organizer', 'Type a collection name first.', show=True)
        return None
    return name
...
class EditSelectedCollectionsDialog(QDialog):
    ...
    def _reload(self) -> None:
        ...
        try:
            ...
            if not out.is_file():
                error_dialog(
                    self, 'Fanfic Organizer', 'Could not explain collection membership.', show=True
                )
                return
            from calibre_plugins.fanfic_organizer.collection_rules import parse_explain
    ...
    def _write_calibre(self) -> None:
        ...
        from calibre_plugins.fanfic_organizer.scrape_run import build_collections_argv
