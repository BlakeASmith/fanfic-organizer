...
calibre-plugin/         # Calibre plugin (search/scrape via ao3kit + JSONL/zip import)
  sources/              # Per-source adapters (AO3, Wikipedia, web): dialog, plan, ids
...
- Put new logic in `scrape` / `identify` / `tags` / `epubs` / `config` / `http` / `rate` / `jobs`, not only in one UI.
...
- Preview tags are `vX.Y.Z-preview.<run>+<sha>` and marked pre-release. PR tags are `vX.Y.Z-pr.<n>+<sha>` and marked pre-release.
...
**Import → TWC journal…** (`calibre-plugin/sources/twc/`, library: `ao3kit.sources.twc` / `python -m ao3kit twc`) imports [Transformative Works and Cultures](https://journal.transformativeworks.org/) Open Journal Systems pages: paste an **issue** URL (every article in that issue) or a single **article** URL. Landing-page citation metadata (author, abstract, keywords) plus the **HTML galley** body become JSONL with `source=twc`, identifier `twc`, and publisher Transformative Works and Cultures; **Build EPUB** (default on) packs each HTML galley and stamps a generated cover.
...
**Fill from AO3** (Selected books) identifies selected library books that are missing an AO3 work id. It reads identifiers and comments, scans native EPUBs for a works URL, and otherwise searches AO3 by title and author. If more than one work matches, a picker asks which one to use. Identified books then scrape work-page metadata (and missing EPUBs, following plugin download/simplify/series settings) onto those same rows — the same update path as import with update-existing. `python -m ao3kit identify` and `scrape --works-from` are the CLI equivalents.
...
**Generate covers** (Selected books) stamps an auto-generated cover (title, author, word count, and quality score on a fandom-coloured field, like [add-cover-to-ao3-files](https://github.com/alexwlchan/add-cover-to-ao3-files)) into each selected EPUB and sets the Calibre thumbnail. Books without an EPUB still get a Calibre cover from library metadata. Style (fields, colours, font, size, layout, contrast) is in Plugin settings → Cover style and `python -m ao3kit config set cover.<key>`. Long titles shrink and tighten instead of truncating. For **omnibus** books, Cover style can also show **member titles** and/or **part numbers** (Combine stamps a cover when covers are enabled).

**Combine selected / series / collection** builds a new **omnibus** library row (identifier `omnibus`) whose EPUB merges member files under stable `m/<id>/…` paths with a hierarchical ToC. Virtual member JSONL lives inside the EPUB (`META-INF/fanfic-organizer/`). Series combine can check AO3 for newer parts first; collection omnibuses can auto-append/shrink when membership changes (Plugin settings master switch). **Edit omnibus…** reorders spine/ToC without rewriting paths, or explodes members back into separate books. `python -m ao3kit epub …` is the CLI.
...
**Plugin settings** hold optional AO3 login and import defaults (download native EPUBs, generate covers, simplify tags/fandoms/relationships, update existing books, **always import the rest of the series**). Cover style (which lines to show, layout, contrast, fandom colour overrides, font, size) is on that page. When that series option is on, Search AO3, Search similar, a series URL, and JSONL/zip import also fetch every other work in the same series (`scrape --include-series` / `--series-from`). Search filters apply only to the original matches; series-mates are added in full. **Test login** on that page checks the username and password against AO3 (via `python -m ao3kit login`) without saving yet. Leave login blank for anonymous access or to use the ao3kit project `.env`. **AO3 request pacing** (min interval between requests and tag-cache warm pause) is stored in XDG `config.yaml` and editable on that page; the host-wide rate limiter still backs off on 429s. Search / Import dialogs still let you override download, simplify, and update-existing for that run. Download native EPUBs and generated covers are on by default; simplification and full-series auto-import stay off unless enabled in settings (or simplify is checked on the dialog).
...
Native AO3 EPUBs have no cover image. `ao3kit.covers` generates one (title, author, word count, and quality score on a dark fandom-coloured gradient, same idea as [add-cover-to-ao3-files](https://github.com/alexwlchan/add-cover-to-ao3-files)) and stamps it into the EPUB OPF. Same-fandom works get the same colour so a grid view groups them. Long titles auto-shrink with tighter line spacing so they stay on the cover. Style lives in `~/.config/fanfic-organizer/config.yaml` under `cover:` (fields, font, size, layout, contrast, palette, per-fandom colour map). Download/scrape use `cover.enabled` (default on). Cover style is in plugin settings; **Selected books → Generate covers** stamps covers onto existing EPUBs.
...
- **Rate limits** — shared slot reservation before each request; honor `429` + `Retry-After` as a host-wide cooldown (every interface waits; cruise intervals stay at the engine floor / tag lane). Work pages, search listings, and EPUB downloads use the host-wide adaptive limiter (default ~1.5s, configurable in Plugin settings / `config.yaml` `min_request_interval`; backs off on 429/pressure). Tag profiles share the same engine and never outrun scrape/search pacing.
