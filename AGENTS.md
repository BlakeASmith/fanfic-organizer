...
webcompile/             # Multi-page web crawl → preprocess → unified EPUB (+ Tampermonkey); not AO3-specific
...
**Import → URL or HTML…** (`calibre-plugin/sources/web/`, library: `ao3kit.sources.web` / `python -m ao3kit web`, multi-page: standalone `webcompile` / `python -m webcompile`) fetches a static HTML URL or reads a browser-exported HTML file, extracts metadata/content best-effort, and builds an EPUB (`source=web`, `web` identifier, publisher Web). **Multi-page compile** prefers the companion **Tampermonkey** userscript (dialog opens the Tampermonkey site, saves the script, and shows install/crawl/export steps); import the exported JSON bundle to build one unified EPUB with a TOC. Optional static seed-URL crawl (free / same-domain / specific-domains) is available for plain HTML only.
