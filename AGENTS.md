...
- **Rate limits** — shared slot reservation before each request; honor `429` + `Retry-After` as a host-wide cooldown (every interface waits; cruise intervals stay at the engine floor / tag lane). Work pages, search listings, and EPUB downloads use the host-wide adaptive limiter (default ~1.0s, backs off on 429/pressure). Tag profiles share the same engine and never outrun scrape/search pacing.
