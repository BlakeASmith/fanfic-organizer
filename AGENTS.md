...
- Preview tags are `vX.Y.Z-preview.<run>+<sha>` and marked pre-release. PR tags are `vX.Y.Z-pr.<n>+<sha>` (also pre-release; public zip asset).
...
The toolbar button opens the plugin menu. The same menu is available from the library **right-click** context menu (placed once on first launch). Menu order (most-used first):

- **Complete selected** / **Fill from AO3** / Download EPUB / Generate covers / Import rest of series / Fill series / Simplify tags / Edit · recompute · add collections / **Search similar…** (disabled when nothing is selected)
- **Search AO3 and import…** / **Process library…** / **Running jobs…**
- **Tags and collections** — Collections & tag rules, tag graph, tag purge, warm / log / stop tag cache
- **Import** — JSONL or zip (files produced outside Calibre)
- **Check for updates…** / **Deploy to KOReader…** / **Plugin settings…**
...
**Fill from AO3** (Selected books) identifies selected library books that are missing an AO3 work id. It reads identifiers and comments, scans native EPUBs for a works URL, and otherwise searches AO3 by title and author. If more than one work matches, a picker asks which one to use. Identified books then scrape work-page metadata (and missing EPUBs, following plugin download/simplify/series settings) onto those same rows as each page lands (EPUBs attach as they download) — the same update path as import with update-existing. `python -m ao3kit identify` and `scrape --works-from` are the CLI equivalents.
