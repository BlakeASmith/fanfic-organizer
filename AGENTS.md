...
ao3kit/                 # Application package
  ...
  sources/              # Multi-source contracts + Wikipedia (MediaWiki) → JSONL
...
calibre-plugin/         # Calibre plugin (search/scrape via ao3kit + JSONL/zip import)
  sources/              # Per-source adapters (AO3, Wikipedia): dialog, plan, ids
...
- **Search AO3 and import…** / **Search Wikipedia and import…** / **Process library…** / **Running jobs…** (toolbar only)
...
**Search Wikipedia and import** (menu) is a separate source adapter under `calibre-plugin/sources/wikipedia/` (library: `ao3kit.sources.wikipedia` / `python -m ao3kit wikipedia`). It writes the same JSONL shape with `source=wikipedia`, a `wikipedia` identifier, and publisher Wikipedia; categories become Tags. No EPUB step. The toolbar menu discovers sources from the registry so AO3 dialogs/job plans stay AO3-only.
