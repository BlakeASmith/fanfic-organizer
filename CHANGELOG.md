...
- Expand **Import → URL or HTML…** with **multi-page compile**: prefer the companion **Tampermonkey** crawler (install link + inline steps in the dialog; export a JSON bundle), then build one unified EPUB with a TOC via the standalone ``webcompile`` package (`python -m webcompile`). Optional static seed-URL crawl (same-domain / domains / free) remains for plain HTML sites.
