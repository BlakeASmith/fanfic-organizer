...
- Fix **Process library…** (and Simplify / Recompute collections) failing at “Writing into Calibre library…” when any book lacks an AO3 work id/URL: ingest now accepts ``calibre_book_id`` / ``calibre_uuid`` on cleaned JSONL rows so local-only books still update.
