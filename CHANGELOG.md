...
- Add a pluggable **content source** layer and **Search Wikipedia and import…**: Wikipedia articles import as Calibre books with a `wikipedia` identifier, Wikipedia publisher, summary, and category tags (`python -m ao3kit wikipedia`). AO3 stays the default source; source-specific UI and matching live under `sources/` rather than in the AO3 dialogs.
