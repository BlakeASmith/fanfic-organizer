...
def test_pr_meta_writes_tag_notes_and_versioned_zip(tmp_path: Path, monkeypatch):
    output = tmp_path / "github-output"
    ...
    assert "download_url=https://github.com/" in text
    assert "%2B" in text  # + in tag/filename encoded for download URL
    body = notes.read_text(encoding="utf-8")
